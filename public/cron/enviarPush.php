    ...
    if ($read->getResult()) {
        ...
        foreach ($read->getResult() as $item) {
            ...
            if(!empty($inscricao[$item['ownerpub']]) && !empty($pushs[$item['notificacao']])) {
                ...
                /**
                 * Para cada inscrição que este usuário tiver, manda para todos os canais
                 */
        ...
        foreach ($webPush->flush() as $report) {
            if (!$report->isSuccess()) {
                /**
                 * Remove user push subscription when error push notification
                 */
                $endpoint = $report->getRequest()->getUri()->__toString();
                $sql = new \Conn\SqlCommand();
                $sql->exeCommand("DELETE FROM " . PRE . "push_notification WHERE JSON_EXTRACT(subscription, '$.endpoint') = '{$endpoint}'");
            }
        }
    }
}