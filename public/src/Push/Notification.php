    ...
    public static function create(string $titulo, string $descricao, $usuarios)
    {
        ...
        if (is_numeric($note) && $note > 0) {
            ...
            if (is_numeric($usuarios)) {
                $read->exeRead("notifications_report", "WHERE ownerpub = :u && notificacao = :n", "u={$usuarios}&n={$note}");
                if (!$read->getResult()) {
                ...
                foreach ($usuarios as $usuario) {
                    if (is_numeric($usuario)) {
                        $read->exeRead("notifications_report", "WHERE ownerpub = :u && notificacao = :n", "u={$usuario}&n={$note}");
                        if (!$read->getResult()) {
                            $create->exeCreate("notifications_report", [
                                "notificacao" => $note,
                                "data_de_envio" => date("Y-m-d H:i:s"),
        ...
        if (is_numeric($note) && $note > 0) {
            if ($this->usuarios !== 0) {
                ...
                if (is_numeric($this->usuarios)) {
                    $read->exeRead("notifications_report", "WHERE ownerpub = :u && notificacao = :n", "u={$this->usuarios}&n={$note}");
                    if(!$read->getResult()) {
                        $create->exeCreate("notifications_report", [
                            "notificacao" => $note,
